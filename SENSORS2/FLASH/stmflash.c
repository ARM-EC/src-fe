...
//    CRC_ResetDR();
//    
//    crc = CRC_CalcBlockCRC((u32*)WriteData,(_len-1)/2);
//    WriteData[_len-1]=crc;
    for(i=0;i<(_len-1);i++)
        crc += WriteData[i];
    
    WriteData[_len-1] = crc;
    STMFLASH_Write(FLASH_SAVE_ADDR,WriteData,_len);//写入数据 
}
...
u8 Flash_Read(u16 *WriteData,u16 _len)   	
{
    u16 crc = 0xA5A5;
    u16 i;
    STMFLASH_Read(FLASH_SAVE_ADDR,WriteData,_len);//读出数据
